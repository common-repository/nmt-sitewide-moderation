...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		...
		if ( action ) {
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', adminurl + 'comment.php?action=un' + action + 'comment&c=' + id + '&_wpnonce=' + settings.data._ajax_nonce);
...
commentReply = {
	...
	open : function(comment_id, post_id, action) {
		...
		act = t.act = act + '-comment-multi'; // magic!!
	...
	send : function() {
		...
		if (! (my_ajaxurl = $('#comment-' + $('#comment_ID').val()).data('ajax')) ) {
			my_ajaxurl = ajaxurl;
		}
